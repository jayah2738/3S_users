...
export default function ParentInfoPage() {
  const { parentMessages, addParentMessage, updateMessage, deleteMessage, isLoading, error } = useMessages();
  const [searchQuery, setSearchQuery] = useState('');
  const [editingMessage, setEditingMessage] = useState<{ id: string; content: string } | null>(null);
  const [newMessage, setNewMessage] = useState('');

  const filteredMessages = parentMessages?.filter(message => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    await addParentMessage(newMessage);
    setNewMessage('');
  };
