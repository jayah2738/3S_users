"use client";
import Image from "next/image";
import { useState } from "react";
import SectionTitle from "../Common/SectionTitle";
import ModalVideo from "react-modal-video";
import { motion } from "framer-motion";
import { fadeIn } from "../variants";
import ReactPlayer from "react-player";

const Video = () => {
  const [isOpen, setOpen] = useState(false);

  return (
    <section className="relative z-10 py-10 md:py-20 lg:py-16">
      <div className="container">
        <SectionTitle
          title="Ready to start learning"
          paragraph="We are here to help you with your learning journey. Our platform offers a wide range of courses and resources to help you achieve your goals"
          center
          mb="80px"
        />

        <motion.div
          variants={fadeIn("up", 0.2)}
          initial="hidden"
          whileInView={"show"}
          viewport={{ once: false, amount: 0.7 }}
          className="-mx-4 flex flex-wrap"
        >
          <div className="w-full px-4">
            <div
              className="mx-auto max-w-[900px] max-h-[500px] overflow-hidden rounded-lg "
              data-wow-delay=".15s"
            >
              <div className="relative aspect-[77/40] items-center justify-center">
                {/* <Image src="/images/video/v1.jpg" alt="video image" fill /> */}
                  <ReactPlayer
                    url="https://www.youtube.com/watch?v=ZcO4KuriDU8"
                    playing        // auto-plays when loaded
                    loop           // repeats forever
                    controls={false}      // shows player controls
                    width="900px"
                    height="500px"
                  />
                {/* <div className="absolute right-0 top-0 flex h-full w-full items-center justify-center">
                  <button
                    aria-label="video play button"
                    onClick={() => setOpen(true)}
                    className="flex h-[70px] w-[70px] items-center justify-center rounded-full bg-white bg-opacity-75 text-amber-500 transition hover:bg-opacity-100"
                  >
                    <svg
                      width="16"
                      height="18"
                      viewBox="0 0 16 18"
                      className="fill-amber-500"
                    >
                      <path d="M15.5 8.13397C16.1667 
                      8.51888 16.1667 9.48112 15.5 
                      9.86602L2 17.6603C1.33333 18.0452 
                      0.499999 17.564 0.499999 16.7942L0.5 
                      1.20577C0.5 0.43597 1.33333 -0.0451549 
                      2 0.339745L15.5 8.13397Z" />
                    </svg>
                  </button>
                </div> */}
              </div>
            </div>
          </div>
        </motion.div>
      </div>

      <ModalVideo
        channel="youtube"
        autoplay={true}
        start={true}
        isOpen={isOpen}
        videoId="ZcO4KuriDU8"
        controls="0"
        onClose={() => setOpen(false)}
      />

      <motion.div
        variants={fadeIn("down", 0.2)}
        initial="hidden"
        whileInView={"show"}
        viewport={{ once: true, amount: 0.7 }}
        className="absolute bottom-0 left-0 right-0 z-[-1] h-full w-full bg-[url(/images/video/shape1.png)] bg-cover bg-center bg-no-repeat">
      </motion.div>
    </section>
  );
};

export default Video;
